    ...
    <x-app-layout>
            <h2 class="font-semibold text-xl text-purple-700 leading-tight slide-in">
                {{ __('Edit Project') }} - <span class="text-purple-600">{{ $project->name }}</span>
            </h2>
                ...
                <div class="form-card overflow-hidden rounded-2xl shadow-xl">
                    <div class="p-8">
                        <form method="POST" action="{{ route('projects.update', $project->id) }}" class="space-y-6">
                            ...
                            <div class="mb-6">
                                <label for="end_date" class="block text-sm font-medium text-purple-800 mb-3">
                                    {{ __('End Date') }}
                                </label>
                                <input
                                    type="date"
                            ...
                            <div class="mb-6">
                                ...
                                >
                                    <option value="todo" {{ old('status', $project->status) == 'todo' ? 'selected' : '' }} class="status-option">📋 Todo</option>
                                    <option value="doing" {{ old('status', $project->status) == 'doing' ? 'selected' : '' }} class="status-option">🚧 Doing</option>
                                    <option value="done" {{ old('status', $project->status) == 'done' ? 'selected' : '' }} class="status-option">✅ Done</option>
                                </select>
                                @error('status')
                                    <p class="mt-2 text-sm text-red-600 animate-pulse">{{ $message }}</p>
                                @enderror
